...
var EditableUserTable = function (id, columns, request) {
        ...
        if (column.editable === true) {
          ...
          if (column.type === "select") {
            ...
            const input = `<input type="${
              column.type
            }" class="h-[32px] max-w-[120px] border border-solid border-gray-300 rounded text-center" value="${$td.text()}">`;
            $td.html(input);
          }
        }
      }
  ...
  this.initHandlers = function () {
    ...
    $(id + " tbody").on(
      "click",
      "button[data-role=action_dropdown]",
      function () {
          ...
          $.post(
            ...
            function (res) {
              ...
              switch (res) {
                ...
                default:
                  toastr.warning(
                    "Please try again after reload this page.",
                    "Warning!"
                  );
                  break;
              }
    ...
    $(id + " tbody").on(
      "click",
      "button[data-role=priority_dropdown]",
      function () {
        const $tr = $(this).closest("tr");
    ...
    $(id + " tbody").on("click", ".priority-dropdown-menu button", function () {
      ...
      const content = `${$(this).attr(
        "data-value"
      )}<svg class="-mr-1 h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd"></path></svg>`;
...
var HomeModule = (function () {
  ...
  var initHandlers = function () {
    $("#run-bot").on("click", function (e) {
      /*
            ...
            */
      if ($("#bottype").is(":checked")) {
        console.log("on");
        socket.send("admin-call-bot:on");
      } else {
  ...
  var getAllUsersData = function () {
    $.get(
      BASE_URL + "controller/homeController.php?action=get-all-users",
      function (res) {
  ...
  var getAllBotsData = function () {
    $.get(
      BASE_URL + "controller/homeController.php?action=get-all-bots",
      function (res) {
...
$(document).ready(function () {
  ...
  // Create editable table for user management
  const userTable = new EditableUserTable(
    "#tbl-users",
    userColumns,
    userRequest
  );
