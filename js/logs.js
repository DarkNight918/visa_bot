...
var LogsModule = (function () {
  ...
  const dateTimeFormat = function (totalMilliseconds) {
    ...
    return `${datetime.getFullYear()}-${
      datetime.getMonth() + 1
    }-${datetime.getDay()} ${datetime.getHours()}:${datetime.getMinutes()}:${datetime.getSeconds()}:${datetime.getMilliseconds()}`;
  };
  ...
  const updateTable = function (data) {
    ...
    const badge =
      '<span class="absolute px-[5px] py-0 ml-3 bg-red-500 top-0 rounded-md text-white text-[12px]">+ new</span>';
